...
def data():
    ...
    for user, dept in query.items:
        ...
        role_names_str = ', '.join(role_names) if role_names else '无'
    ...
    return table_api(data=result_data, count=query.total)
...
def save():
    ...
    # 处理角色分配
    if a and a.strip():
        role_ids = [rid.strip() for rid in a.split(',') if rid.strip()]
        if role_ids:
...
def edit(id):
    ...
    roles = Role.query.all()
    checked_roles = []
    # user.role是lazy='dynamic'关系，需要使用.all()获取
...
def update_avatar():
    ...
    db.session.commit()
    if not r:
        admin_log(request, is_access=False, desc="用户管理-修改头像-失败")
        return fail_api(msg="出错啦")
    admin_log(request, is_access=True, desc="用户管理-修改头像-成功")
...
def update_info():
    ...
    db.session.commit()
    if not r:
        admin_log(request, is_access=False, desc="用户管理-修改个人信息-失败")
        return fail_api(msg="出错啦")
    admin_log(request, is_access=True, desc="用户管理-修改个人信息-成功")
    return success_api(msg="更新成功")
...
# 设置用户密码（管理员为其他用户设置密码）
@bp.get('/setPassword/<int:id>')
@authorize("system:user:edit", log=True)
def set_password(id):
    user = curd.get_one_by_id(User, id)
    return render_template('system/user/set_password.html', user=user)


# 设置用户密码（管理员为其他用户设置密码）
@bp.put('/setPassword')
@authorize("system:user:edit", log=True)
def set_password_put():
    res_json = request.get_json(force=True)
    user_id = res_json.get("userId")
    new_password = str_escape(res_json.get("newPassword"))
    
    if not user_id:
        return fail_api(msg="用户ID不能为空")
    
    if not new_password or new_password == '':
        return fail_api(msg="新密码不得为空")
    
    if len(new_password) < 6 or len(new_password) > 12:
        return fail_api(msg="密码长度必须在6-12位之间")
